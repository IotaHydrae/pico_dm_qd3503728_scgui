...
#else
#define SC_SCREEN_WIDTH	 (480)
#define SC_SCREEN_HEIGHT (320)
#define SC_PFB_BUF_SIZE \
	(SC_SCREEN_WIDTH * SC_SCREEN_HEIGHT / 2) // 示例：10行高度
#define SC_LCD_DMA_2BUF (1) // 是否启用DMA双buf传输
#define SC_LCD_DMA_WAP	(0) // 是否DMA传输时高低位WAP
#endif

/* 脏区管理配置，单桶8字节*/
#define SC_DIRTY_BUCKET_N \
	(8) // 脏桶数量，用满强制合并到第一个，多个控件同时更新时适当改大
#define SC_DIRTY_AREA_THRESHOLD \
	(15 * 15) // 脏桶相交大于阀值面积合并，脏桶数量剩余一半时为0
#define SC_DIRTY_BUCKET_COPY (0) // 脏桶临时拷贝，默认:0不拷贝

/*lvgl字体配置*/
#define SC_UINCODE_U16 \
	(1) // 0：生僻汉字使用32位编码，默认:1常用字使用16位编码节省堆栈空间
...
#define SC_ALIGN_ARRAY(type, name, size, boundary) \
	_Pragma("data_alignment=" #boundary) type name[size]
...
/*矩形rect相交输出dirty区域*/
static inline bool sc_rect_intersect_to_area(const sc_rect_t *a,
					     const sc_rect_t *b,
					     sc_dirty_t *out)
{
...
/*pfb相交判断*/
bool sc_pfb_intersection(const sc_pfb_t *dest, const sc_rect_t *b,
			 sc_dirty_t *out);

/*矩形对齐计算*/
void sc_parent_align(sc_rect_t *parent, int *xs, int *ys, int width, int height,
		     sc_align_t align);
